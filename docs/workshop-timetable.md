...
| — | 10 min | 1:10 | **休憩** |
| 6 | 20 min | 1:30 | ハンズオン①: 要件一覧を作る（`sdd-requirements`） |
| 7 | 20 min | 1:50 | ハンズオン②: 設計書を作る（`sdd-design`） |
| 8 | 15 min | 2:05 | ハンズオン③: ワイヤーフレームを作る（`sdd-wireframe`） |
| 9 | 10 min | 2:15 | ハンズオン④: Azure への CI/CD を作る（`sdd-pipeline`、デモ中心） |
...
- ハンズオンで使う 4 エージェント（`sdd-requirements` / `sdd-design` / `sdd-wireframe` / `sdd-pipeline`）の役割と、エージェントの呼び出し方 2 通りを紹介。
...
### 9. ハンズオン④: Azure への CI/CD を作る（`sdd-pipeline`）（10 min）

- ③で作成した `app/index.html` を入力に `@sdd-pipeline` で GitHub Actions ワークフローを生成。
